                        ...
                        with save_col2:
                            if st.button("Save Hierarchical Cluster Assignments"):
                                try:
                                    ...
                                    filepath = feature_engineer.save_features(
                                        assignments_df, "hierarchical_cluster_assignments", base_path_save, "csv"
                                    )
                                ...
                                try:
                                    with st.spinner("Calculating LOS and comparing across clusters..."):
                                        ...
                                        if len(unique_clusters) < 2:
                                             ...
                                             if p_val < 0.05: st.success("Significant difference in LOS found (p < 0.05).")
                                             else: st.info("No significant difference in LOS found (p >= 0.05).")
                                        else:
                                             ...
                                             if p_val < 0.05: st.success("Significant difference in LOS found across clusters (p < 0.05).")
                                             else: st.info("No significant difference in LOS found across clusters (p >= 0.05).")
...
class MIMICDashboardApp:
    def __init__(self):
        ...
        self.clustering_analyzer = MIMICClusteringAnalysis() # Handles running algorithms, evaluation
        self.cluster_analyzer    = MIMICClusterAnalyzer()    # Handles post-cluster analysis, interpretation
    ...
    def run(self):
        ...
        if st.session_state.current_view == 'data_explorer':
            ...
            self.filtering_tab.render() # Assuming FilteringTab uses session_state or needs no args
    ...
    def _display_sidebar(self):
        ...
        selected_view = st.sidebar.radio(
             ...
             key="view_selector" # Add key for stability
        )
        ...
        if st.session_state.available_tables:
            ...
            if module in st.session_state.available_tables:
                table_options = sorted(st.session_state.available_tables[module]) # Sort alphabetically
                ...
                # Determine index for selected table
                selected_table_index = 0
                if st.session_state.selected_table and st.session_state.selected_table in table_options:
                     # Find the display name corresponding to the stored table name
                     for i, tbl in enumerate(table_options):
                          if tbl == st.session_state.selected_table:
                ...
                if table != st.session_state.selected_table:
                     ...
                     st.session_state.df = None # Clear dataframe if table changes


                # Show table info (description)
                if st.session_state.selected_table:
                     try:
                          table_info = self.data_handler.get_table_info(module, st.session_state.selected_table)
                          if table_info:
                ...
                sample_size_disabled = load_full
                st.session_state.sample_size = st.sidebar.number_input(
                    "Sample Size (Rows)",
                    min_value=100, max_value=2000000, # Increased max sample size
                    value=st.session_state.sample_size, step=100,
                    disabled=sample_size_disabled,
