					...
					if k_max <= k_min:
						...
						try:
							with st.spinner(f"Calculating Elbow and Silhouette scores for k={k_min} to {k_max}..."):
								...
								inertia_scores, silhouette_scores = self.clustering_analyzer.find_optimal_k_kmeans_elbow_silhouette(
									data_for_clustering,
									k_range=k_range,
									n_init=n_init,
									max_iter=max_iter
					...
					if k_dist >= len(data_for_clustering):
						...
						try:
							with st.spinner(f"Calculating {k_dist}-distance graph..."):
								...
								suggested_eps, k_distances_sorted = self.clustering_analyzer.find_optimal_eps_for_dbscan(
									data_for_clustering,
									k_dist=k_dist,
