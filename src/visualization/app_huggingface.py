...
class MIMICDataHandler:
        ...
        for module in modules:
            ...
            if os.path.exists(module_path):
                ...
                for file_path in all_files:
                    ...
                    table_name = os.path.basename(file_path).replace(
                        ".csv.gz" if is_gz else ".csv", ""
                    )
                ...
                for table_name, file_path in current_module_tables.items():
                    ...
                    table_display_names[(module, table_name)] = (
                        f"{table_name} {size_str}"
                    )
    ...
    def load_mimic_table(
        ...
        try:
            ...
            logging.error(
                f"Error loading data from {os.path.basename(file_path)}: {str(e)}"
            )
            return None
    ...
    def convert_to_parquet(
        self, df: pd.DataFrame, table_name: str, current_file_path: str
    ) -> Optional[str]:
        """Converts the loaded DataFrame to Parquet format. Returns path or None."""
        try:
            parquet_dir = os.path.join(
                os.path.dirname(current_file_path), "parquet_files"
            )
...
class MIMICVisualizer:
    ...
    def display_dataset_statistics(self, df: Optional[pd.DataFrame]):
        ...
        if df is not None:
            st.markdown(
                "<h2 class='sub-header'>Dataset Statistics</h2>", unsafe_allow_html=True
            )
    ...
    def display_data_preview(self, df: Optional[pd.DataFrame]):
        if df is not None:
            st.markdown(
                "<h2 class='sub-header'>Data Preview</h2>", unsafe_allow_html=True
            )
    ...
    def display_visualizations(self, df: Optional[pd.DataFrame]):
        if df is not None:
            st.markdown(
                "<h2 class='sub-header'>Data Visualization</h2>", unsafe_allow_html=True
            )
            ...
            if len(numeric_cols) > 0:
                st.markdown(
                    "<h3>Numeric Data Visualization</h3>", unsafe_allow_html=True
                )

                # Histogram
                selected_num_col = st.selectbox(
                    "Select a numeric column for histogram", numeric_cols
                )
                if selected_num_col:
                    fig = px.histogram(
                        df,
            ...
            if len(categorical_cols) > 0:
                st.markdown(
                    "<h3>Categorical Data Visualization</h3>", unsafe_allow_html=True
                )

                # Bar chart
                selected_cat_col = st.selectbox(
                    "Select a categorical column for bar chart", categorical_cols
                )
                if selected_cat_col:
                    ...
                    fig = px.bar(
                        value_counts, x=selected_cat_col, y="Count", title=title
                    )
                    st.plotly_chart(fig, use_container_width=True)
...
class MIMICFeatureEngineer:
    ...
    def detect_order_columns(self, df: pd.DataFrame) -> List[str]:
        ...
        for col in df.columns:
            ...
            # Or if column has common order-related suffixes/prefixes
                ...
                or col_lower.startswith("order_")
            ):
                order_columns.append(col)
        ...
        for col in df.columns:
            ...
            if any(term in col_lower for term in time_related_terms):
                ...
                if df[col].dtype == "datetime64[ns]":
                    ...
                    if any(
                        sample.str.contains(pattern).any() for pattern in date_patterns
                    ):
                        time_columns.append(col)
    ...
    def create_order_frequency_matrix(
        self,
        df: pd.DataFrame,
        patient_id_col: str,
        order_col: str,
        ...
        if patient_id_col not in df.columns or order_col not in df.columns:
            raise ValueError(
                f"Columns {patient_id_col} or {order_col} not found in DataFrame"
            )
        ...
        freq_matrix = pd.crosstab(filtered_df[patient_id_col], filtered_df[order_col])
    ...
    def extract_temporal_order_sequences(
        self,
        df: pd.DataFrame,
        patient_id_col: str,
        order_col: str,
        ...
        if not all(col in df.columns for col in [patient_id_col, order_col, time_col]):
            missing = [
                col
                for col in [patient_id_col, order_col, time_col]
    ...
    def create_order_timing_features(
        ...
        for patient_id, patient_data in grouped:
            ...
            if discharge_time_col and discharge_time_col in df.columns:
                ...
                time_to_discharge = (
                    discharge_time - last_order_time
                ).total_seconds() / 3600
                features["time_from_last_order_to_discharge_hours"].append(
                    time_to_discharge
                )
    ...
    def get_order_type_distributions(
        self, df: pd.DataFrame, patient_id_col: str, order_col: str
        ...
        if not all(col in df.columns for col in [patient_id_col, order_col]):
            missing = [
                col for col in [patient_id_col, order_col] if col not in df.columns
            ]
            raise ValueError(f"Columns {missing} not found in DataFrame")
        ...
        if patient_dists:
            ...
            patient_level_dist = pd.DataFrame(
                columns=[order_col, "frequency", "patient_id"]
            )

        return overall_dist, patient_level_dist

    def calculate_order_transition_matrix(
        self, sequences: Dict[Any, List[str]], top_n: int = 20
    ) -> pd.DataFrame:
        """
    ...
    def save_features(
        self, features: Any, feature_type: str, base_path: str, format: str = "csv"
    ) -> str:
        """
...
class MIMICDashboardApp:
    ...
    def run(self):
        ...
        # Set page config
        st.set_page_config(
            page_title="MIMIC-IV Explorer", page_icon="🏥", layout="wide"
        )
    ...
    def _display_sidebar(self):
        ...
        if st.sidebar.button("Scan MIMIC-IV Directory"):
            if not mimic_path or mimic_path == "/path/to/mimic-iv-3.1":
                ...
                available_tables, file_paths, file_sizes, table_display_names = (
                    self.data_handler.scan_mimic_directory(mimic_path)
                )
                ...
                if available_tables:
                    ...
                    st.sidebar.success(
                        f"Found {sum(len(tables) for tables in available_tables.values())} tables in {len(available_tables)} modules"
                    )
                else:
        ...
        if st.session_state.available_tables:
            ...
            if module in st.session_state.available_tables:
                ...
                table_display_options = [
                    st.session_state.table_display_names.get((module, table), table)
                ...
                display_to_table = {
                    display: table
                    for table, display in zip(table_options, table_display_options)
                }
                ...
                with st.sidebar.expander("Advanced Options"):
                    ...
                    st.session_state.sample_size = st.number_input(
                        "Sample Size", 100, 10000, 1000, 100
                    )
                ...
                if st.sidebar.button("Load Table"):
                    ...
                    if file_path:
                        ...
                        df = self.data_handler.load_mimic_table(
                            file_path=file_path,
                        ...
                        if df is not None:
                            ...
                            st.session_state.detected_order_cols = (
                                self.feature_engineer.detect_order_columns(df)
                            )
                            st.session_state.detected_time_cols = (
                                self.feature_engineer.detect_temporal_columns(df)
                            )
                            st.session_state.detected_patient_id_col = (
                                self.feature_engineer.detect_patient_id_column(df)
                            )
    ...
    def _display_main_content_with_tabs(self):
        ...
        if st.session_state.df is not None:
            # Dataset info
            st.markdown(
                "<h2 class='sub-header'>Dataset Information</h2>",
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                f"**File:** {os.path.basename(st.session_state.current_file_path)}"
            )

            # Get file size and format it
            file_size_mb = st.session_state.file_sizes.get(
                (st.session_state.selected_module, st.session_state.selected_table), 0
            )
            if file_size_mb < 1:
            ...
            st.markdown(
                f"**Sample Size:** {min(len(st.session_state.df), st.session_state.sample_size)} rows out of {len(st.session_state.df)}"
            )  # TODO: Need total row count
            st.markdown("</div>", unsafe_allow_html=True)

            # Create tabs
            tab1, tab2, tab3 = st.tabs(
                ["Exploration & Visualization", "Feature Engineering", "Export Options"]
            )
            ...
            with tab3:
                st.markdown(
                    "<h2 class='sub-header'>Export Options</h2>", unsafe_allow_html=True
                )
                col1, col2 = st.columns(2)
                    ...
                    if st.button("Convert to Parquet"):
                        try:
                            ...
                            parquet_file = os.path.join(
                                parquet_dir,
                                f"{st.session_state.selected_table}.parquet",
                            )
                            ...
                            st.success(
                                f"Dataset converted to Parquet format at {parquet_file}"
                            )
                        except Exception as e:
    ...
    def _display_feature_engineering_tab(self):
        ...
        st.markdown(
            "<h2 class='sub-header'>Order Data Feature Engineering</h2>",
            unsafe_allow_html=True,
        )
        ...
        with feature_tabs[0]:
            st.markdown(
                "<h3>Create Order Frequency Matrix</h3>", unsafe_allow_html=True
            )
            st.markdown(
                """
			<div class='info-box'>
            ...
            with col1:
                ...
                if st.session_state.detected_patient_id_col in all_columns:
                    default_patient_idx = all_columns.index(
                        st.session_state.detected_patient_id_col
                    )
            ...
            with col2:
                ...
                if (
                    st.session_state.detected_order_cols
                    and st.session_state.detected_order_cols[0] in all_columns
                ):
                    default_order_idx = all_columns.index(
                        st.session_state.detected_order_cols[0]
                    )
            ...
            if st.button("Generate Order Frequency Matrix"):
                try:
                    with st.spinner("Generating order frequency matrix..."):
                        freq_matrix = (
                            self.feature_engineer.create_order_frequency_matrix(
                                st.session_state.df,
                                patient_id_col,
                                order_col,
                                normalize,
            ...
            if st.session_state.freq_matrix is not None:
                ...
                # Show preview
                st.dataframe(
                    st.session_state.freq_matrix.head(10), use_container_width=True
                )

                # Matrix stats
                st.markdown(
                    f"<div class='info-box'>Matrix size: {st.session_state.freq_matrix.shape[0]} patients × {st.session_state.freq_matrix.shape[1]} order types</div>",
                    unsafe_allow_html=True,
                )

                # Heatmap visualization
                st.markdown(
                    "<h4>Frequency Matrix Heatmap (Sample)</h4>", unsafe_allow_html=True
                )
                ...
                matrix_sample = st.session_state.freq_matrix.iloc[
                    :sample_size, :sample_cols
                ]
                ...
                with save_col1:
                    save_format = st.radio(
                        "Save Format", ["CSV", "Parquet"], horizontal=True
                    )
                with save_col2:
        ...
        with feature_tabs[1]:
            st.markdown(
                "<h3>Extract Temporal Order Sequences</h3>", unsafe_allow_html=True
            )
            st.markdown(
                """
			<div class='info-box'>
            ...
            with col3:
                ...
                if (
                    st.session_state.detected_time_cols
                    and st.session_state.detected_time_cols[0] in all_columns
                ):
                    default_time_idx = all_columns.index(
                        st.session_state.detected_time_cols[0]
                    )
            ...
            if st.button("Extract Order Sequences"):
                try:
                    with st.spinner("Extracting temporal order sequences..."):
                        sequences = (
                            self.feature_engineer.extract_temporal_order_sequences(
                                st.session_state.df,
                                seq_patient_id_col,
                                seq_order_col,
                                seq_time_col,
                        ...
                        transition_matrix = (
                            self.feature_engineer.calculate_order_transition_matrix(
                                sequences, top_n=15  # Limit to top 15 for visualization
                            )
                        )
            ...
            if st.session_state.order_sequences is not None:
                ...
                avg_sequence_length = np.mean(
                    [len(seq) for seq in st.session_state.order_sequences.values()]
                )
                ...
                for patient in sample_patients:
                    ...
                    st.markdown(
                        f"<strong>Patient {patient}:</strong> {sequence_str}",
                        unsafe_allow_html=True,
                    )
                ...
                if st.session_state.transition_matrix is not None:
                    st.markdown(
                        "<h4>Order Transition Matrix</h4>", unsafe_allow_html=True
                    )
                    st.markdown(
                        """
					<div class='info-box'>
        ...
        with feature_tabs[2]:
            st.markdown(
                "<h3>Analyze Order Type Distributions</h3>", unsafe_allow_html=True
            )
            st.markdown(
                """
			<div class='info-box'>
            ...
            if st.button("Analyze Order Distributions"):
                try:
                    with st.spinner("Analyzing order type distributions..."):
                        overall_dist, patient_dist = (
                            self.feature_engineer.get_order_type_distributions(
                                st.session_state.df, dist_patient_id_col, dist_order_col
                            )
                        )
            ...
            if st.session_state.order_dist is not None:
                ...
                st.markdown(
                    "<h4>Overall Order Type Distribution</h4>", unsafe_allow_html=True
                )
                ...
                if len(st.session_state.order_dist) > top_n_orders:
                    others_sum = st.session_state.order_dist.iloc[top_n_orders:][
                        "frequency"
                    ].sum()
                    other_row = pd.DataFrame(
                        {dist_order_col: ["Other"], "frequency": [others_sum]}
                    )
                ...
                if (
                    st.session_state.patient_order_dist is not None
                    and not st.session_state.patient_order_dist.empty
                ):
                    st.markdown(
                        "<h4>Patient-Level Order Type Distribution</h4>",
                        unsafe_allow_html=True,
                    )

                    # Get unique patients
                    patients = st.session_state.patient_order_dist[
                        "patient_id"
                    ].unique()
                    ...
                    fig = make_subplots(
                        ...
                        cols=1,
                        subplot_titles=[
                            f"Patient {patient}" for patient in sample_patients
                        ],
                    )
                    ...
                    for i, patient in enumerate(sample_patients):
                        patient_data = st.session_state.patient_order_dist[
                            ...
                            10
                        )  # Top 10 orders for this patient
                    ...
                    fig.update_layout(
                        height=200 * len(sample_patients), showlegend=False
                    )
                    st.plotly_chart(fig, use_container_width=True)
                ...
                save_col1, save_col2 = st.columns(2)
                with save_col1:
                    dist_save_format = st.radio(
                        "Save Format",
                        ["CSV", "Parquet"],
                        horizontal=True,
                        key="dist_save_format",
                    )
                with save_col2:
        ...
        with feature_tabs[3]:
            ...
            if st.button("Generate Timing Features"):
                try:
                    with st.spinner("Generating order timing features..."):
                        timing_features = (
                            self.feature_engineer.create_order_timing_features(
                                st.session_state.df,
            ...
            if st.session_state.timing_features is not None:
                ...
                st.dataframe(
                    st.session_state.timing_features.head(10), use_container_width=True
                )

                # Generate visualizations based on available features
                st.markdown(
                    "<h4>Order Timing Visualizations</h4>", unsafe_allow_html=True
                )

                numeric_cols = st.session_state.timing_features.select_dtypes(
                    include=["number"]
                ).columns
                ...
                if "total_orders" in st.session_state.timing_features.columns:
                    ...
                    with col2:
                        ...
                        if (
                            "unique_order_types"
                            in st.session_state.timing_features.columns
                        ):
                ...
                if (
                    ...
                    with col2:
                        ...
                        if all(
                            col in st.session_state.timing_features.columns
                            for col in [
                                "orders_in_first_24h",
                                "orders_in_first_48h",
                                "orders_in_first_72h",
                ...
                save_col1, save_col2 = st.columns(2)
                with save_col1:
                    timing_save_format = st.radio(
                        "Save Format",
                        ["CSV", "Parquet"],
                        horizontal=True,
                        key="timing_save_format",
                    )
                with save_col2:
